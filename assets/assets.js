...
categoriesToPopulate.forEach((cat) => {
  for (let i = 1; i <= 20; i++) {
    // Deterministic price based on category and index to prevent hydration errors
    const basePrice = (cat.length * 150) + (i * 75) + 500;
    const discountAmount = (i * 10) % 100 + 50; 
    
    productsDummyData.push({
      _id: `gen_${cat.replace(/[^a-zA-Z0-9]/g, "")}_${i}`,
      userId: "user_2sZFHS1UIIysJyDVzCpQhUhTIhw",
      ...
      category: cat,
      date: 1715100000000 + (i * 86400000), // Stable daily increment
      __v: 0,
    });
  }
});
